...
Bitfield world;
Bitfield world_target;
TransformedView tv;

char World_Get(Bitfield* world,int x,int y){
...
void Update(AlxWindow* w){
	...
	if(Stroke(ALX_KEY_SPACE).DOWN){
		...
		Bitfield_Memcpy(&world,&world_target);
	}
...
void Delete(AlxWindow* w){
	Bitfield_Free(&world);
	Bitfield_Free(&world_target);
}

int main(){
    if(Create("Cellular Automa Bitfield",2200,1200,1,1,Setup,Update,Delete))
        Start();
    return 0;
}